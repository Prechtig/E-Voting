package org.evoting.security;

import java.math.BigInteger;
import java.security.SecureRandom;
import org.bouncycastle.crypto.AsymmetricCipherKeyPair;
import org.bouncycastle.crypto.DataLengthException;
import org.bouncycastle.crypto.engines.ElGamalEngine;
import org.bouncycastle.crypto.generators.ElGamalKeyPairGenerator;
import org.bouncycastle.crypto.generators.ElGamalParametersGenerator;
import org.bouncycastle.crypto.params.ElGamalKeyGenerationParameters;
import org.bouncycastle.crypto.params.ElGamalParameters;
import org.bouncycastle.crypto.params.ElGamalPrivateKeyParameters;
import org.bouncycastle.crypto.params.ElGamalPublicKeyParameters;
import org.bouncycastle.crypto.params.ParametersWithRandom;
import org.bouncycastle.util.Arrays;
import org.bouncycastle.util.encoders.Hex;
import org.bouncycastle.util.test.SimpleTest;

public class ElGamalTest extends SimpleTest {
	private BigInteger g512 = new BigInteger("153d5d6172adb43045b68ae8e1de1070b6137005686d29d3d73a7749199681ee5b212c9b96bfdcfa5b20cd5e3fd2044895d609cf9b410b7a0f12ca1cb9a428cc", 16);
	private BigInteger p512 = new BigInteger("9494fec095f3b85ee286542b3836fc81a5dd0a0349b4c239dd38744d488cf8e31db8bcb7d33b41abb9e5a33cca9144b1cef332c94bf0573bf047a3aca98cdf3b", 16);
	private BigInteger g768 = new BigInteger("7c240073c1316c621df461b71ebb0cdcc90a6e5527e5e126633d131f87461c4dc4afc60c2cb0f053b6758871489a69613e2a8b4c8acde23954c08c81cbd36132cfd64d69e4ed9f8e51ed6e516297206672d5c0a69135df0a5dcf010d289a9ca1", 16);
	private BigInteger p768 = new BigInteger("8c9dd223debed1b80103b8b309715be009d48860ed5ae9b9d5d8159508efd802e3ad4501a7f7e1cfec78844489148cd72da24b21eddd01aa624291c48393e277cfc529e37075eccef957f3616f962d15b44aeab4039d01b817fde9eaa12fd73f", 16);
	private BigInteger g1024 = new BigInteger("", 16);
	private BigInteger p1024 = new BigInteger("", 16);
	private BigInteger yPgpBogusPSamp = new BigInteger("", 16);
	private BigInteger xPgpBogusPSamp = new BigInteger("", 16);
	private BigInteger gPgpBogusPSamp = new BigInteger("", 16);
	private BigInteger pPgpBogusPSamp = new BigInteger("", 16);
	private int lPgpBogusPSamp = 0;

	public String getName() {
		return "ElGamal";
	}

	private void testEnc(int paramInt1, int paramInt2, BigInteger paramBigInteger1, BigInteger paramBigInteger2) {
		ElGamalParameters localElGamalParameters = new ElGamalParameters(paramBigInteger2, paramBigInteger1, paramInt2);
		ElGamalKeyGenerationParameters localElGamalKeyGenerationParameters = new ElGamalKeyGenerationParameters(new SecureRandom(), localElGamalParameters);
		ElGamalKeyPairGenerator localElGamalKeyPairGenerator = new ElGamalKeyPairGenerator();
		localElGamalKeyPairGenerator.init(localElGamalKeyGenerationParameters);
		AsymmetricCipherKeyPair localAsymmetricCipherKeyPair = localElGamalKeyPairGenerator.generateKeyPair();
		ElGamalPublicKeyParameters localElGamalPublicKeyParameters = (ElGamalPublicKeyParameters) localAsymmetricCipherKeyPair.getPublic();
		ElGamalPrivateKeyParameters localElGamalPrivateKeyParameters = (ElGamalPrivateKeyParameters) localAsymmetricCipherKeyPair.getPrivate();
		checkKeySize(paramInt2, localElGamalPrivateKeyParameters);
		ElGamalEngine localElGamalEngine = new ElGamalEngine();
		localElGamalEngine.init(true, localElGamalPublicKeyParameters);
		if (localElGamalEngine.getOutputBlockSize() != paramInt1 / 4) {
			fail(paramInt1 + " getOutputBlockSize() on encryption failed.");
		}
		byte[] arrayOfByte1 = Hex.decode("5468697320697320612074657374");
		byte[] arrayOfByte2 = arrayOfByte1;
		byte[] arrayOfByte3 = localElGamalEngine.processBlock(arrayOfByte2, 0, arrayOfByte2.length);
		localElGamalEngine.init(false, localElGamalPrivateKeyParameters);
		if (localElGamalEngine.getOutputBlockSize() != paramInt1 / 8 - 1) {
			fail(paramInt1 + " getOutputBlockSize() on decryption failed.");
		}
		arrayOfByte2 = localElGamalEngine.processBlock(arrayOfByte3, 0, arrayOfByte3.length);
		if (!Arrays.areEqual(arrayOfByte1, arrayOfByte2)) {
			fail(paramInt1 + " bit test failed");
		}
		localElGamalEngine.init(true, localElGamalPublicKeyParameters);
		byte[] arrayOfByte4 = new byte[localElGamalEngine.getInputBlockSize() + 2];
		try {
			localElGamalEngine.processBlock(arrayOfByte4, 0, arrayOfByte4.length);
			fail("out of range block not detected");
		} catch (DataLengthException localDataLengthException1) {
		}
		try {
			arrayOfByte4[0] = -1;
			localElGamalEngine.processBlock(arrayOfByte4, 0, arrayOfByte4.length - 1);
			fail("out of range block not detected");
		} catch (DataLengthException localDataLengthException2) {
		}
		try {
			arrayOfByte4[0] = 127;
			localElGamalEngine.processBlock(arrayOfByte4, 0, arrayOfByte4.length - 1);
		} catch (DataLengthException localDataLengthException3) {
			fail("in range block failed");
		}
	}

	private void checkKeySize(int paramInt, ElGamalPrivateKeyParameters paramElGamalPrivateKeyParameters) {
		if ((paramInt != 0) && (paramElGamalPrivateKeyParameters.getX().bitLength() != paramInt)) {
			fail("limited key check failed for key size " + paramInt);
		}
	}

	private void testGeneration(int paramInt) {
		ElGamalParametersGenerator localElGamalParametersGenerator = new ElGamalParametersGenerator();
		localElGamalParametersGenerator.init(paramInt, 10, new SecureRandom());
		ElGamalParameters localElGamalParameters = localElGamalParametersGenerator.generateParameters();
		if (localElGamalParameters.getL() != 0) {
			fail("ElGamalParametersGenerator failed to set L to 0 in generated ElGamalParameters");
		}
		ElGamalKeyGenerationParameters localElGamalKeyGenerationParameters = new ElGamalKeyGenerationParameters(new SecureRandom(), localElGamalParameters);
		ElGamalKeyPairGenerator localElGamalKeyPairGenerator = new ElGamalKeyPairGenerator();
		localElGamalKeyPairGenerator.init(localElGamalKeyGenerationParameters);
		AsymmetricCipherKeyPair localAsymmetricCipherKeyPair = localElGamalKeyPairGenerator.generateKeyPair();
		ElGamalPublicKeyParameters localElGamalPublicKeyParameters = (ElGamalPublicKeyParameters) localAsymmetricCipherKeyPair.getPublic();
		ElGamalPrivateKeyParameters localElGamalPrivateKeyParameters = (ElGamalPrivateKeyParameters) localAsymmetricCipherKeyPair.getPrivate();
		ElGamalEngine localElGamalEngine = new ElGamalEngine();
		localElGamalEngine.init(true, new ParametersWithRandom(localElGamalPublicKeyParameters, new SecureRandom()));
		byte[] arrayOfByte1 = Hex.decode("5468697320697320612074657374");
		byte[] arrayOfByte2 = arrayOfByte1;
		byte[] arrayOfByte3 = localElGamalEngine.processBlock(arrayOfByte2, 0, arrayOfByte2.length);
		localElGamalEngine.init(false, localElGamalPrivateKeyParameters);
		arrayOfByte2 = localElGamalEngine.processBlock(arrayOfByte3, 0, arrayOfByte3.length);
		if (!Arrays.areEqual(arrayOfByte1, arrayOfByte2)) {
			fail("generation test failed");
		}
	}

	private void testInitCheck() {
		try {
			new ElGamalEngine().processBlock(new byte[] { 1 }, 0, 1);
			fail("failed initialisation check");
		} catch (IllegalStateException localIllegalStateException) {
		}
	}

	private void testInvalidP() {
		ElGamalParameters localElGamalParameters = new ElGamalParameters(this.pPgpBogusPSamp, this.gPgpBogusPSamp, this.lPgpBogusPSamp);
		ElGamalPublicKeyParameters localElGamalPublicKeyParameters = new ElGamalPublicKeyParameters(this.yPgpBogusPSamp, localElGamalParameters);
		ElGamalPrivateKeyParameters localElGamalPrivateKeyParameters = new ElGamalPrivateKeyParameters(this.xPgpBogusPSamp, localElGamalParameters);
		ElGamalEngine localElGamalEngine = new ElGamalEngine();
		localElGamalEngine.init(true, localElGamalPublicKeyParameters);
		byte[] arrayOfByte1 = Hex.decode("5468697320697320612074657374");
		byte[] arrayOfByte2 = arrayOfByte1;
		byte[] arrayOfByte3 = localElGamalEngine.processBlock(arrayOfByte2, 0, arrayOfByte2.length);
		localElGamalEngine.init(false, localElGamalPrivateKeyParameters);
		arrayOfByte2 = localElGamalEngine.processBlock(arrayOfByte3, 0, arrayOfByte3.length);
		if (Arrays.areEqual(arrayOfByte1, arrayOfByte2)) {
			fail("invalid test failed");
		}
	}

	public void performTest() {
		testInvalidP();
		testEnc(512, 0, this.g512, this.p512);
		testEnc(768, 0, this.g768, this.p768);
		testEnc(1024, 0, this.g1024, this.p1024);
		testEnc(512, 64, this.g512, this.p512);
		testEnc(768, 128, this.g768, this.p768);
		testGeneration(258);
		testInitCheck();
	}

	public static void main(String[] paramArrayOfString) {
		runTest(new ElGamalTest());
	}
}
